...
exports.createUser = async (req, res) => {
    const { first_name, last_name, password, role, user_code, card_code, permissions, departments } = req.body;
  
    try {
      const existing = await User.findOne({ user_code });
      if (existing) return res.status(400).json({ message: 'Bu user_code allaqachon mavjud' });
